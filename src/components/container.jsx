...
const Container = () =>{
    ...
    const getWeather = async (e) => {
        ...
        const getCity = await fetch(url + location + "&appid=" + key);
        var weatherData = await getCity.json();
        ...
        <section className="h-auto w-auto bg-white rounded-2xl shadow-2xl shadow-white opacity-40 p-5">
            ...
            <div className="w-full h-auto">
                <article className="w-full p-3 text-center">
                    <p className="font-poppins text-7xl font-semibold my-2">{weather && weather.main && Math.round(weather.main.temp)}°C</p>
                    <p className="font-poppins text-3xl">{weather && weather.name}</p>
                </article>
                <section className="w-full flex flex-row py-10 px-5">
                    <div className="flex mr-10">
                        <div><WiHumidity size={80}/></div>
                        <div className="py-2">
                            <p className="font-poppins text-xl">{weather && weather.main.humidity}%</p>
                    ...
                    <div className="flex ml-10">
                        ...
                        <div className="py-2 ml-2">
                            <p className="font-poppins text-xl">{weather && weather.wind.speed}km/h</p>
